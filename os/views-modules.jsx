...
function QbConnectModal({onClose}){
  const t=DRtok;
  const canOAuth = window.DRQuickBooks && DRQuickBooks.canOAuth && DRQuickBooks.canOAuth();
  const [company,setCompany]=useStateM('');
  const QbMark=({s=18})=>(<span style={{display:'inline-grid',placeItems:'center',width:s,height:s,borderRadius:5,background:'#2CA01C',color:'#fff',fontSize:s*0.5,fontWeight:800,fontFamily:t.fHead,flex:'none'}}>qb</span>);
  const inp={width:'100%',border:`1px solid ${t.line}`,borderRadius:9,padding:'10px 12px',fontSize:13,fontFamily:t.fBody,outline:'none'};
  const lbl={fontSize:11.5,fontWeight:700,color:t.muted,display:'block',marginBottom:5};
  const steps=[['1','In QuickBooks Online, confirm the company you bill from.'],['2','Connect here — invoices, payments and bills map to it.'],['3','Use “Sync to QuickBooks” to push current invoices to your books.']];
  return (
    <div onClick={onClose} style={{position:'fixed',inset:0,background:'rgba(14,26,40,.82)',backdropFilter:'blur(3px)',WebkitBackdropFilter:'blur(3px)',zIndex:1200,display:'grid',placeItems:'center',padding:24}}>
      <div onClick={e=>e.stopPropagation()} style={{width:520,maxWidth:'100%',background:'#fff',borderRadius:18,boxShadow:'0 24px 60px rgba(13,30,48,.4)',fontFamily:t.fBody}}>
        <div style={{background:'linear-gradient(135deg,#1a7a14,#2CA01C)',padding:'20px 24px',color:'#fff',display:'flex',alignItems:'center',gap:13}}>
          <div style={{width:40,height:40,borderRadius:11,background:'#fff',display:'grid',placeItems:'center',flex:'none'}}><QbMark s={22}/></div>
          <div style={{flex:1}}><div style={{fontFamily:t.fHead,fontWeight:800,fontSize:17}}>QuickBooks Online</div><div style={{fontSize:12.5,color:'#D6F0D0',marginTop:2}}>Keep your books current — sync invoices &amp; payments.</div></div>
        </div>
        <div style={{padding:'22px 24px'}}>
          {canOAuth ? (
            <React.Fragment>
              <button onClick={()=>{ DRQuickBooks.connectOAuth(); onClose(); }} style={{width:'100%',display:'flex',alignItems:'center',justifyContent:'center',gap:9,background:'#2CA01C',color:'#fff',border:0,borderRadius:10,padding:'13px',fontSize:14,fontWeight:700,cursor:'pointer'}}><QbMark s={16}/>Connect to QuickBooks</button>
              <div style={{display:'flex',alignItems:'center',gap:10,margin:'16px 0'}}><span style={{flex:1,height:1,background:t.lineSoft}}/><span style={{fontSize:10.5,fontWeight:700,color:t.faint,textTransform:'uppercase',letterSpacing:'.06em'}}>or name it manually</span><span style={{flex:1,height:1,background:t.lineSoft}}/></div>
            </React.Fragment>
          ) : (
            <div style={{fontSize:12,color:t.muted,background:t.cardTint,border:`1px solid ${t.line}`,borderRadius:9,padding:'10px 12px',marginBottom:14,lineHeight:1.45}}>Real Intuit sign-in turns on once a QuickBooks client ID is added in <b style={{fontFamily:t.fMono,color:t.ink}}>os/connect-config.js</b> (token exchange runs server-side). For now, name your company to connect.</div>
          )}
          <label style={lbl}>QuickBooks company name</label>
          <input value={company} onChange={e=>setCompany(e.target.value)} placeholder="DunRite Construction Group LLC" style={inp}/>
          <div style={{display:'flex',gap:10,marginTop:16}}>
            <button onClick={()=>{ DRQuickBooks.connect({company:company.trim()||'QuickBooks Online'}); onClose(); }} style={{flex:1,background:'#2CA01C',color:'#fff',border:0,borderRadius:10,padding:'12px',fontSize:13.5,fontWeight:700,cursor:'pointer'}}>Connect</button>
          ...
          <div style={{marginTop:18,paddingTop:16,borderTop:`1px solid ${t.lineSoft}`}}>
            <div style={{display:'flex',flexDirection:'column',gap:9}}>
              {steps.map(([n,txt])=>(<div key={n} style={{display:'flex',gap:10,alignItems:'flex-start'}}><span style={{width:20,height:20,borderRadius:6,background:'rgba(44,160,28,.10)',color:'#2CA01C',fontWeight:800,fontSize:11,display:'grid',placeItems:'center',flex:'none'}}>{n}</span><span style={{fontSize:12.5,color:t.ink2,lineHeight:1.4}}>{txt}</span></div>))}
            </div>
          </div>
        </div>
      </div>
...
function InvoicesView({user}){
  ...
  const [qb,setQb]=useStateM(window.DRQuickBooks?DRQuickBooks.status:'off');
  const [qbInfo,setQbInfo]=useStateM(window.DRQuickBooks?DRQuickBooks.info:{});
  const [qbForm,setQbForm]=useStateM(false);
  React.useEffect(()=>{ if(window.DRQuickBooks) return DRQuickBooks.onStatus((s,i)=>{setQb(s);setQbInfo(i||{});}); },[]);
  const qbOn=qb==='connected';
  const unsynced=inv.filter(v=>!v.qbSynced).length;
  const QbMark=({s=18})=>(<span style={{display:'inline-grid',placeItems:'center',width:s,height:s,borderRadius:5,background:'#2CA01C',color:'#fff',fontSize:s*0.5,fontWeight:800,fontFamily:t.fHead,flex:'none'}}>qb</span>);
  return (
    <div>
      <PageHead title="Invoices" sub="Billing across all jobs · updates cash position as they're paid"
        right={qbOn && <button onClick={()=>drToast(DROS.actions.syncInvoicesToQB(user))} style={{display:'inline-flex',alignItems:'center',gap:8,background:'#2CA01C',color:'#fff',border:0,borderRadius:10,padding:'10px 16px',fontSize:13,fontWeight:700,cursor:'pointer'}}><QbMark s={16}/>Sync to QuickBooks{unsynced?` · ${unsynced}`:''}</button>}/>
      <div style={{display:'flex',alignItems:'center',gap:13,padding:'12px 15px',borderRadius:12,marginBottom:16,border:`1px solid ${qbOn?'rgba(44,160,28,.30)':t.line}`,background:qbOn?'rgba(44,160,28,.05)':t.cardTint}}>
        <div style={{width:38,height:38,borderRadius:10,background:qbOn?'rgba(44,160,28,.12)':'#EEF2F6',display:'grid',placeItems:'center',flex:'none'}}><QbMark s={20}/></div>
        <div style={{flex:1,minWidth:0}}>
          <div style={{fontSize:13.5,fontWeight:700,color:t.ink}}>{qbOn?`Connected to QuickBooks · ${qbInfo.company||'QuickBooks Online'}`:'QuickBooks not connected'}</div>
          <div style={{fontSize:12,color:t.muted,marginTop:1}}>{qbOn?`Invoices push to your books${qbInfo.synced?` · ${qbInfo.synced} synced`:''}.`:'Connect QuickBooks Online to sync invoices, bills and payments to your books.'}</div>
        </div>
        {qbOn
          ? <button onClick={()=>DRQuickBooks.disconnect()} style={{background:'#fff',color:t.bad,border:`1px solid ${t.line}`,borderRadius:9,padding:'9px 14px',fontSize:12.5,fontWeight:700,cursor:'pointer',flex:'none'}}>Disconnect</button>
          : <button onClick={()=>setQbForm(true)} style={{display:'inline-flex',alignItems:'center',gap:7,background:'#2CA01C',color:'#fff',border:0,padding:'9px 14px',borderRadius:9,fontSize:12.5,fontWeight:700,cursor:'pointer',flex:'none'}}><QbMark s={15}/>Connect QuickBooks</button>}
      </div>
      {qbForm && (window.Portal ? <Portal><QbConnectModal onClose={()=>setQbForm(false)}/></Portal> : <QbConnectModal onClose={()=>setQbForm(false)}/>)}
          ...
          <tbody>
            {inv.map(v=>{ const [sl,sk]=stMap[v.status];
              return <tr key={v.id} style={{borderBottom:`1px solid ${t.lineSoft}`}}>
                ...
                <td style={{padding:'12px 16px'}}><div style={{display:'flex',alignItems:'center',gap:8}}><Pill kind={sk}>{sl}</Pill>{qbOn && v.qbSynced && <span title="Synced to QuickBooks" style={{display:'inline-flex',alignItems:'center',gap:4,fontSize:10.5,fontWeight:800,color:'#2CA01C',background:'rgba(44,160,28,.10)',borderRadius:6,padding:'2px 7px'}}><QbMark s={12}/>Synced</span>}</div></td>
