...
/* ---- portal: lets fixed-position modals escape the shell's stacking
   contexts (the topbar/content columns) and render at the document root
   so they always sit above the app instead of behind it. ---- */
...
function SyncBadge(){
  ...
  return <React.Fragment>
    ...
    {open && <Portal><ConnectModal status={s} onClose={()=>setOpen(false)}/></Portal>}
  </React.Fragment>;
}
...
function DropboxBadge(){
  ...
  return <React.Fragment>
    ...
    {open && <Portal><DropboxModal onClose={()=>setOpen(false)}/></Portal>}
  </React.Fragment>;
}
...
function DropboxModal({onClose}){
  ...
  const steps=[
    ['1','In Dropbox, open the team folder that holds your DunRite jobs.'],
  ...
  return (
    <div onClick={onClose} style={{position:'fixed',inset:0,background:'rgba(14,26,40,.82)',backdropFilter:'blur(3px)',WebkitBackdropFilter:'blur(3px)',zIndex:1200,display:'grid',placeItems:'center',padding:24}}>
      <div onClick={e=>e.stopPropagation()} style={{width:560,maxWidth:'100%',maxHeight:'90vh',overflowY:'auto',background:'#fff',borderRadius:18,boxShadow:'0 24px 60px rgba(13,30,48,.4)',fontFamily:t.fBody}}>
        ...
        <div style={{padding:'22px 24px'}}>
          {(window.DRDropbox&&DRDropbox.canChoose&&DRDropbox.canChoose()) ? (
            <React.Fragment>
              <button onClick={async()=>{ const r=await DRDropbox.chooseFolder(); if(r&&r.ok) onClose(); }} style={{width:'100%',display:'flex',alignItems:'center',justifyContent:'center',gap:9,background:'#0061FF',color:'#fff',border:0,borderRadius:10,padding:'13px',fontSize:14,fontWeight:700,cursor:'pointer'}}><DropboxLogo s={17} color="#fff"/>Connect with Dropbox</button>
              <div style={{display:'flex',alignItems:'center',gap:10,margin:'16px 0'}}><span style={{flex:1,height:1,background:t.lineSoft}}/><span style={{fontSize:10.5,fontWeight:700,color:t.faint,textTransform:'uppercase',letterSpacing:'.06em'}}>or paste a link</span><span style={{flex:1,height:1,background:t.lineSoft}}/></div>
            </React.Fragment>
          ) : (
            <div style={{fontSize:12,color:t.muted,background:t.cardTint,border:`1px solid ${t.line}`,borderRadius:9,padding:'10px 12px',marginBottom:14,lineHeight:1.45}}>One-click connect turns on once a Dropbox app key is added in <b style={{fontFamily:t.fMono,color:t.ink}}>os/connect-config.js</b>. For now, paste a folder link below.</div>
          )}
...
/* ---- Microsoft 365 connect badge + modal (OneDrive / SharePoint) ---- */
const MicrosoftLogo = ({s=16, style}) => (
  <svg viewBox="0 0 23 23" style={{width:s,height:s,flex:'none',...style}} aria-hidden="true">
...
function MicrosoftBadge(){
  const t=DRtok; const [s,setS]=useState(window.DRMicrosoft?DRMicrosoft.status:'off'); const [open,setOpen]=useState(false);
  useEffect(()=>{ if(window.DRMicrosoft) return DRMicrosoft.onStatus(setS); },[]);
  const live = s==='connected';
  const c = live?'#0067B8':t.idle, bg = live?'rgba(0,103,184,.10)':t.idleBg;
  return <React.Fragment>
    <button onClick={()=>setOpen(true)} title="Connect Microsoft 365" style={{display:'inline-flex',alignItems:'center',gap:7,height:40,padding:'0 12px',borderRadius:10,background:bg,color:c,fontSize:12.5,fontWeight:700,border:0,cursor:'pointer',flex:'none'}}>
      <MicrosoftLogo s={15}/>365
    </button>
    {open && <Portal><MicrosoftModal onClose={()=>setOpen(false)}/></Portal>}
  </React.Fragment>;
}

function MicrosoftModal({onClose}){
  const t=DRtok;
  const [url,setUrl]=useState(window.DRMicrosoft?DRMicrosoft.info.url:'');
  const [s,setS]=useState(window.DRMicrosoft?DRMicrosoft.status:'off');
  const [info,setInfo]=useState(window.DRMicrosoft?DRMicrosoft.info:{});
  useEffect(()=>{ if(window.DRMicrosoft) return DRMicrosoft.onStatus((st,inf)=>{setS(st);setInfo(inf||{});}); },[]);
  const live = s==='connected';
  const canSignIn = window.DRMicrosoft && DRMicrosoft.canSignIn && DRMicrosoft.canSignIn();
  const connect=()=>{ if(!url.trim())return; DRMicrosoft.connect({url:url.trim()}); };
  const disconnect=()=>{ DRMicrosoft.disconnect(); setUrl(''); };
  const inp={width:'100%',border:`1px solid ${t.line}`,borderRadius:9,padding:'10px 12px',fontSize:12.5,fontFamily:t.fMono,outline:'none'};
  const lbl={fontSize:11.5,fontWeight:700,color:t.muted,display:'block',marginBottom:5};
  const when = info&&info.when ? new Date(info.when).toLocaleDateString([], {month:'short',day:'numeric',hour:'numeric',minute:'2-digit'}) : null;
  const steps=[
    ['1','In OneDrive or SharePoint, open the team folder for your DunRite jobs.'],
    ['2','Click Share \u2192 Copy link (or grab the document-library URL).'],
    ['3','Paste it below and connect \u2014 the OS document folders map to it.'],
    ['4','Files filed in the OS mirror into Microsoft 365; \u201cOpen in 365\u201d jumps straight there.'],
  ];
  return (
    <div onClick={onClose} style={{position:'fixed',inset:0,background:'rgba(14,26,40,.82)',backdropFilter:'blur(3px)',WebkitBackdropFilter:'blur(3px)',zIndex:1200,display:'grid',placeItems:'center',padding:24}}>
      <div onClick={e=>e.stopPropagation()} style={{width:560,maxWidth:'100%',maxHeight:'90vh',overflowY:'auto',background:'#fff',borderRadius:18,boxShadow:'0 24px 60px rgba(13,30,48,.4)',fontFamily:t.fBody}}>
        <div style={{background:`linear-gradient(135deg,${t.navyDeep},${t.navy})`,padding:'20px 24px',color:'#fff',display:'flex',alignItems:'center',gap:13,position:'sticky',top:0,zIndex:1}}>
          <div style={{width:40,height:40,borderRadius:11,background:'#fff',display:'grid',placeItems:'center',flex:'none'}}><MicrosoftLogo s={21}/></div>
          <div style={{flex:1}}><div style={{fontFamily:t.fHead,fontWeight:800,fontSize:17}}>Microsoft 365</div><div style={{fontSize:12.5,color:'#9FB3C6',marginTop:2}}>{live?`Connected \u2014 files back up to ${info.folder||'OneDrive'}.`:'Back up every job\u2019s documents to OneDrive or SharePoint.'}</div></div>
          {live && <div style={{fontSize:11,fontWeight:800,letterSpacing:'.05em',textTransform:'uppercase',color:t.ok,background:'rgba(255,255,255,.08)',padding:'5px 10px',borderRadius:20}}>linked</div>}
        </div>
        <div style={{padding:'22px 24px'}}>
          {!live && (canSignIn ? (
            <React.Fragment>
              <button onClick={async()=>{ const r=await DRMicrosoft.signIn(); if(r&&r.ok) onClose(); }} style={{width:'100%',display:'flex',alignItems:'center',justifyContent:'center',gap:9,background:'#0067B8',color:'#fff',border:0,borderRadius:10,padding:'13px',fontSize:14,fontWeight:700,cursor:'pointer'}}><MicrosoftLogo s={16}/>Sign in with Microsoft</button>
              <div style={{display:'flex',alignItems:'center',gap:10,margin:'16px 0'}}><span style={{flex:1,height:1,background:t.lineSoft}}/><span style={{fontSize:10.5,fontWeight:700,color:t.faint,textTransform:'uppercase',letterSpacing:'.06em'}}>or paste a link</span><span style={{flex:1,height:1,background:t.lineSoft}}/></div>
            </React.Fragment>
          ) : (
            <div style={{fontSize:12,color:t.muted,background:t.cardTint,border:`1px solid ${t.line}`,borderRadius:9,padding:'10px 12px',marginBottom:14,lineHeight:1.45}}>One-click sign-in turns on once a Microsoft (Azure) client ID is added in <b style={{fontFamily:t.fMono,color:t.ink}}>os/connect-config.js</b>. For now, paste a OneDrive/SharePoint link below.</div>
          ))}
          <label style={lbl}>OneDrive / SharePoint folder link</label>
          <input value={url} onChange={e=>setUrl(e.target.value)} placeholder="https://yourorg.sharepoint.com/sites/…" style={inp}/>
          {live && <div style={{fontSize:12,color:'#0067B8',background:'rgba(0,103,184,.07)',borderRadius:9,padding:'9px 11px',marginTop:10,display:'flex',alignItems:'center',gap:8}}><MicrosoftLogo s={15}/>Backing up to <b>{info.folder}</b>{when?` \u00b7 since ${when}`:''}</div>}
          <div style={{display:'flex',gap:10,marginTop:16}}>
            <button onClick={connect} style={{flex:1,background:'#0067B8',color:'#fff',border:0,borderRadius:10,padding:'12px',fontSize:13.5,fontWeight:700,cursor:'pointer'}}>{live?'Update link':'Connect Microsoft 365'}</button>
            {live && <a href={info.url||'#'} target="_blank" rel="noopener" style={{display:'inline-flex',alignItems:'center',gap:7,background:'#fff',color:'#0067B8',border:`1px solid ${t.line}`,borderRadius:10,padding:'12px 16px',fontSize:13.5,fontWeight:700,textDecoration:'none'}}><DIcon d={IC.external} s={15}/>Open</a>}
            {live && <button onClick={disconnect} style={{background:'#fff',color:t.bad,border:`1px solid ${t.line}`,borderRadius:10,padding:'12px 16px',fontSize:13.5,fontWeight:700,cursor:'pointer'}}>Disconnect</button>}
            {!live && <button onClick={onClose} style={{background:'#fff',color:t.muted,border:`1px solid ${t.line}`,borderRadius:10,padding:'12px 16px',fontSize:13.5,fontWeight:600,cursor:'pointer'}}>Close</button>}
          </div>
          <div style={{marginTop:18,paddingTop:16,borderTop:`1px solid ${t.lineSoft}`}}>
            <div style={{fontSize:11.5,fontWeight:800,letterSpacing:'.04em',textTransform:'uppercase',color:t.faint,marginBottom:10}}>How to connect</div>
            <div style={{display:'flex',flexDirection:'column',gap:9}}>
              {steps.map(([n,txt])=>(<div key={n} style={{display:'flex',gap:10,alignItems:'flex-start'}}><span style={{width:20,height:20,borderRadius:6,background:'rgba(0,103,184,.10)',color:'#0067B8',fontWeight:800,fontSize:11,display:'grid',placeItems:'center',flex:'none'}}>{n}</span><span style={{fontSize:12.5,color:t.ink2,lineHeight:1.4}}>{txt}</span></div>))}
            </div>
          </div>
        </div>
      </div>
...
function SheetBadge(){
  ...
  return <React.Fragment>
    ...
    {open && <Portal><SheetModal onClose={()=>setOpen(false)}/></Portal>}
  </React.Fragment>;
}
  ...
  return (
    <div onClick={onClose} style={{position:'fixed',inset:0,background:'rgba(14,26,40,.82)',backdropFilter:'blur(3px)',WebkitBackdropFilter:'blur(3px)',zIndex:1200,display:'grid',placeItems:'center',padding:24}}>
      <div onClick={e=>e.stopPropagation()} style={{width:560,maxWidth:'100%',maxHeight:'90vh',overflowY:'auto',background:'#fff',borderRadius:18,boxShadow:'0 24px 60px rgba(13,30,48,.4)',fontFamily:t.fBody}}>
        ...
        <div style={{padding:'22px 24px'}}>
          {(window.DROSheets&&DROSheets.canPick&&DROSheets.canPick()) ? (
            <React.Fragment>
              <button onClick={async()=>{ const r=await DROSheets.pickSheet(); if(r&&r.ok) onClose(); }} style={{width:'100%',display:'flex',alignItems:'center',justifyContent:'center',gap:9,background:t.blue,color:'#fff',border:0,borderRadius:10,padding:'13px',fontSize:14,fontWeight:700,cursor:'pointer'}}><DIcon d={IC.grid} s={17}/>Choose from Google Drive</button>
              <div style={{display:'flex',alignItems:'center',gap:10,margin:'16px 0'}}><span style={{flex:1,height:1,background:t.lineSoft}}/><span style={{fontSize:10.5,fontWeight:700,color:t.faint,textTransform:'uppercase',letterSpacing:'.06em'}}>or paste a link</span><span style={{flex:1,height:1,background:t.lineSoft}}/></div>
            </React.Fragment>
          ) : (
            <div style={{fontSize:12,color:t.muted,background:t.cardTint,border:`1px solid ${t.line}`,borderRadius:9,padding:'10px 12px',marginBottom:14,lineHeight:1.45}}>One-click picking turns on once a Google client ID + API key are added in <b style={{fontFamily:t.fMono,color:t.ink}}>os/connect-config.js</b>. For now, paste a sheet link below.</div>
          )}
...
const VIEW_ART = {
  ...
  commitments: {icon:IC.doc},
  directory:   {icon:IC.user},
  bidding:     {icon:IC.inbox},
  meetings:    {icon:IC.list},
...
function Backdrop({mode, view}){
  ...
  const cy=DRtok.cyan, nv=DRtok.navy;
  const grid=`linear-gradient(${cy}0d 1px, transparent 1px), linear-gradient(90deg, ${cy}0d 1px, transparent 1px)`;
  return <div aria-hidden="true" style={{position:'absolute',inset:0,zIndex:0,pointerEvents:'none',overflow:'hidden'}}>
    {/* base wash */}
    <div style={{position:'absolute',inset:0,background:`linear-gradient(168deg, #FFFFFF 0%, ${DRtok.paper} 46%, #ECF2F8 100%)`}}/>
    {/* hero photo across the top of the dashboard */}
    {art.photo && <div className="dr-kb" style={{position:'absolute',top:0,left:0,right:0,height:388,backgroundImage:'url(assets/site-backdrop.png)',backgroundSize:'cover',backgroundPosition:'center 56%',opacity:strong?0.5:0.34,filter:'grayscale(14%) saturate(.94)',WebkitMaskImage:'linear-gradient(180deg, #000 0%, rgba(0,0,0,.5) 54%, transparent 100%)',maskImage:'linear-gradient(180deg, #000 0%, rgba(0,0,0,.5) 54%, transparent 100%)',animation:'drKenBurns 46s ease-in-out infinite'}}/>}
    {art.photo && <div style={{position:'absolute',top:0,left:0,right:0,height:388,background:`linear-gradient(180deg, ${DRtok.navyDeep}1a 0%, ${cy}0f 36%, transparent 78%)`}}/>}
    {/* blueprint grid, fading in from the top-right */}
    <div style={{position:'absolute',inset:0,backgroundImage:grid,backgroundSize:'48px 48px',opacity:strong?0.6:0.42,WebkitMaskImage:'radial-gradient(135% 95% at 84% -10%, #000 0%, transparent 72%)',maskImage:'radial-gradient(135% 95% at 84% -10%, #000 0%, transparent 72%)'}}/>
    {/* soft brand glows for depth */}
    <div style={{position:'absolute',right:'-7%',top:'-15%',width:560,height:560,borderRadius:'50%',background:`radial-gradient(circle, ${cy}26 0%, transparent 64%)`}}/>
    <div style={{position:'absolute',left:'-9%',bottom:'-18%',width:520,height:520,borderRadius:'50%',background:`radial-gradient(circle, ${nv}14 0%, transparent 66%)`}}/>
    {/* watermark for the non-photo views */}
    {!art.photo && <div style={{position:'absolute',right:-34,bottom:-26,width:400,height:400,backgroundImage:'url(assets/dc-watermark.png)',backgroundSize:'contain',backgroundRepeat:'no-repeat',backgroundPosition:'bottom right',opacity:strong?0.07:0.05,transform:'rotate(-6deg)'}}/>}
  </div>;
}
    ...
    <div style={{display:'grid',gridTemplateColumns:'264px 1fr',height:'100vh',fontFamily:t.fBody,color:t.ink,background:t.paper,overflow:'hidden'}}>
      <aside style={{background:`linear-gradient(180deg,${t.navyDeep},#0A1420)`,display:'flex',flexDirection:'column',borderRight:'1px solid rgba(255,255,255,.05)',overflow:'hidden'}}>
        ...
        <div style={{flex:1,overflowY:'auto',padding:'0 14px',minHeight:0}}>
          ...
          <nav style={{display:'flex',flexDirection:'column',gap:2}}>{(user.role==='owner'?[NAV_MAIN_R[0],{id:'advisor',label:'Advisor',icon:IC.trend},...NAV_MAIN_R.slice(1)]:NAV_MAIN_R).map(n=><ShellNavRow key={n.id} {...n} on={curId===n.id} onClick={()=>go({view:n.id})}/>)}</nav>
      ...
      <div style={{display:'flex',flexDirection:'column',overflow:'hidden',position:'relative'}}>
        ...
        <div style={{padding:'24px 32px 48px',overflowY:'auto',flex:1,position:'relative',zIndex:1}}>
          <div key={route.view+'/'+(route.pid||'')} className="dr-view">
...
function Content({user,route,go}){
  switch(route.view){
    ...
    case 'commitments': return <CommitmentsView user={user}/>;
    case 'directory': return <DirectoryView user={user}/>;
    case 'bidding': return <BiddingView user={user}/>;
    case 'schedule': return <ScheduleView/>;
    case 'automations': return <AutomationsView/>;
    case 'documents': return <DocumentsView user={user}/>;
    case 'inspections': return <InspectionsView user={user}/>;
    case 'meetings': return <MeetingsView user={user}/>;
    ...
    case 'insurance': return <InsuranceView user={user}/>;
